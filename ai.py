...
class FiveGoAi():
	...
	def calNum(self, goType , dx, dy ,x ,y):
		...
		n=self.n
		m=self.m
		score=(n-x)*(m-y)*x*y//n+int(random()*2)%3
		x1=copy(x)
		y1=copy(y)
		num=0
		kong=0
	...
	def search(self,x,y,level,goType):
		n=self.n
		m=self.m
		if level==1 :
		...
		while (i<n):
			...
			while (j<m):
				if (self.goMap[i][j]==-1) :
					...
					if goType==self.goType :
						score=max(score,self.search(i,j,level-1,not goType))
					else :
						score=min(score,self.search(i,j,level-1,not goType))
					self.goMap[i][j]=-1
				j=j+1
			i=i+1
		return score
		...
		while (i<n):
			...
			while (j<m):
				if (self.goMap[i][j]==-1) :
					...
					x=self.search(i,j,2,not self.goType)
					if score<x:
						self.step=(i,j)
						score=x
					self.goMap[i][j]=-1
